...
percent_area_top6 <- webs %>%
  ...
  summarise(
    ...
    total_area_top6 = sum(AREA[adm0_a3 %in% top_countries$adm0_a3], na.rm = TRUE)
  ) %>%
...
## ***********************************************
not_in_analysis <- setdiff(unique(webs_complete$NAME), unique(webs_country$NAME))
...
latex_table <- make_latex_country_table(sort(not_in_analysis, decreasing =FALSE))
cat(latex_table)
...
# 
#
#could have been fine, but we really don't expect webs by country to be log relationship 
#be too odd to explain, plus the fit is crazy

# webs_country$Total_webs_by_country_log <- log(webs_country$Total_webs_by_country + .01)
# library(lme4)
# 
...
library(lmerTest)
M1_lmm <- lmerTest::lmer(log_pub_count ~ Continent * log_years_since_pub + (1 | Web_Code_base),
               data = webs_reuse)
